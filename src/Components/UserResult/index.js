  ...
  return (
    <>
      {QuestionData.map((data, quesIndex) => (
        <div className= {`result-page-question ${selectedAnswers[quesIndex].answer === data["correct-answer"] ? "green" : "red"}`}>
          <div>Question No. {data.id}</div>
          <div className="question-center">
            <div className="question-head" style={{border: "none", }}>{data.question}</div>
          ...
          <div>Correct Answer : {data["correct-answer"]}</div>
        </div>
      ))}
    </>
  );
